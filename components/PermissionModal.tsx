...
const PermissionModal = ({ onClose, link }:PermissionModalProp) => {
  return (
    <div className='w-full h-full fixed inset-0 flex items-center justify-center bg-white/15 bg-opacity-50 backdrop-blur-sm z-50 px-10'>
        <main className='w-full flex flex-col items-center px-5 py-8 bg-white rounded-3xl gap-y-8'>
            ...
            <section className="uppercase text-[#121113] w-full flex justify-between items-center">
                <h2 onClick={onClose}>cancel</h2>
                <span className='py-2 px-6 rounded-full bg-[#FDD902]'><Link href={link}>grant permission</Link></span>
            </section>
        </main>
    </div>
  )
